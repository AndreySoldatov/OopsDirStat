...
async fn main() {
    ...
    if args.len() < 2 || !Path::new(&args[1]).exists() {
        loop {
            ...
            draw_text(
                ...
                screen_width() / 2.0 - get_text_center("Please provide a valid path!", None, 32, 1.0, 0.0).x,
                screen_height() / 2.0 - get_text_center("Please provide a valid path!", None, 32, 1.0, 0.0).y, 
                32.0, 
                GRAY
            );
    ...
    loop {
        let text = format!("Loading stats of {} | {:.1}s", &args[1], get_time() - time_start);
